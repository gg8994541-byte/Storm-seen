...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_PATH)
    curr = conn.execute("SELECT user_id FROM users WHERE user_id=?", (u.id,)).fetchone()
    
    if not curr:
        ref_id = None
        if ctx.args and ctx.args[0].startswith("ref"):
            try:
                potential_ref = int(ctx.args[0].replace("ref", ""))
                if potential_ref != u.id:
                    ref_id = potential_ref
            except:
                ref_id = None
        
        conn.execute("INSERT INTO users (user_id, username, full_name, referred_by) VALUES (?,?,?,?)",
                     (u.id, u.username, u.full_name, ref_id))
        
        if ref_id:
            update_user_balance(ref_id, REFERRAL_BONUS)
            try:
                await ctx.bot.send_message(ref_id, f"🎁 <b>Yangi referal!</b>\nHisobingizga {REFERRAL_BONUS} so'm qo'shildi.", parse_mode="HTML")
            except: pass
        conn.commit()
    conn.close()
    await update.message.reply_text(f"Assalomu alaykum, {u.full_name}!", reply_markup=main_kb())
